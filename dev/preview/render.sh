#!/usr/bin/env bash
#
# Build the preview container and render jovial's palette previews to PNG/GIF
# under dev/preview/output/. Requires only Docker on the host.
#
# Usage:
#   dev/preview/render.sh            # render light, dark, and the detect check
#   dev/preview/render.sh light      # render a single tape
#
...
for mode in light dark detect; do
    ...
    echo "==> rendering ${mode} -> dev/preview/output/${mode}.{gif,png}"
    ...
    docker run --rm -v "${repo}:/work" -w /work --entrypoint vhs "${image}" \
        "/work/dev/preview/${mode}.tape"
    # grab the final frame as a still PNG for quick visual review
    docker run --rm -v "${repo}:/work" -w /work --entrypoint bash "${image}" -c \
        "ffmpeg -y -sseof -0.1 -i '/work/dev/preview/output/${mode}.gif' -update 1 -frames:v 1 '/work/dev/preview/output/${mode}.png' 2>/dev/null"
done

echo "==> done. see dev/preview/output/"
